function [signal, thetas, phis] = mycwepr(Sys, Exp)
    ...
    gs = sqrt( sum(  (gTensor*rVers).^2,   1) );

    % The linewidth are square, following Zech's convention
    cwlwpp = (mt2mhz(Sys.cwlwpp)*1e-3)^2;
    ...
    for ii = 1:nSolidAngles
        ...
        signal(ii, :) = gaussianresonancebsweep( ...
            omReson*1e-3, mwFreq, cwlwpp, "lwpp");
    end
    % Apply strain
    if isfield(Sys, "gStrain")
        ...
        for ii = 1:nSolidAngles
            if gStrainEff(ii) > gs(ii)/1e8
                gBroadeningMt = mhz2mt(...
                    gvalue2freq(mean(Exp.x), gStrainEff(ii)));
                signal(ii, :) = convspec( ...
                    signal(ii, :), xstep, gBroadeningMt);
            end
        end
    end
end
