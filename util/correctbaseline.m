function [y, b, p] = correctbaseline(x, y, varargin)
% correctbaseline      Baseline correction with a polynomial function.
%
% [y, b, p] = correctbaseline(x, y, Opt)
% Input:
...
if isreal(y)
    ...
    % Polynomial fit
    [p, ~, mu] = polyfit(x(idxArray), y(idxArray), Opt.polyOrder);
    ...
    yr = real(y);
    yi = imag(y);
    [yr, br] = correctbaseline(x, yr, Opt);
    [yi, bi] = correctbaseline(x, yi, Opt);
    y = yr + yi*1i;
    b = br + bi*1i;
end
...
% Define parameters.
addParameter(parser, 'polyOrder', 0);

fieldnameArray = fieldnames(varargin{:});
containsOrder = contains('order', lower(fieldnameArray));
if containsOrder
    error("Opt structure contains field 'order'. Change it to 'polyOrder'")
end
