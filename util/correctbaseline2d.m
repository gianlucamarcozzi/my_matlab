function [yfin, ymid, b] = correctbaseline2d(x, y, Opt, dimFirstCorr)
% subtractbaseline      Baseline correction with a polynomial function.
%
% [y, b, p] = subtractBaseline(x, y, Opt, dimFirstCorr)
% Input:
...
if corr1 == 1
    ...
    nSlice = size(y, corr2);
    for iSlice = 1:nSlice
        [ymid(:, iSlice), b{1}(:, iSlice)] = correctbaseline(...
            x{corr1}, y(:, iSlice), Opt{corr1});
    end
    % Correction along second dimension
    nSlice = size(y, corr1);
    for iSlice = 1:nSlice
        [yfin(iSlice, :), b{2}(iSlice, :)] = correctbaseline(...
...
elseif corr1 == 2
    ...
    nSlice = size(y, corr1);
    for iSlice = 1:nSlice
        [ymid, b{1}] = correctbaseline(...
            x{corr2}, y(iSlice, :), Opt{corr2});
    end
    % Correction along first dimension
    nSlice = size(y, corr2);
    for iSlice = 1:nSlice
        [yfin, b{2}] = correctbaseline(...
